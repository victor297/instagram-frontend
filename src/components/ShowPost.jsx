...
function ShowPost(props) {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      await axiosJWT.post(`${backendapi}/comment/`, submitData, {
  ...
  useEffect(() => {
    const fetchComment = async () => {
      const res = await axios.get(`${backendapi}/comment/${props.post._id}`);

      res.data.comments.forEach(async (comment) => {
        const res = await axios.get(`${backendapi}/user/${comment.user}`);
